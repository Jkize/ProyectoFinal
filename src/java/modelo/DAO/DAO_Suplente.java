...
public class DAO_Suplente implements DAO<Suplente> {
    ...
    public Suplente buscar(Object correo) throws FileNotFoundException, IOException {
        ...
        if (pos != -1) {
            archivo.seek(pos);
            Suplente sup = new Suplente(archivo.readUTF(), archivo.readUTF(), archivo.readUTF(), archivo.readUTF(), (new DAO_Operador()).buscar(archivo.readUTF()), archivo.readUTF(), archivo.readUTF());
            if (sup.getCargo().equals("Suplente")) {
                return sup;
            }
        }
